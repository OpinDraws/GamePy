...
from entities.weapons.player_weapons import SlashProjectile 
import rendering
from systems.skills import SkillManager

class Player(pygame.sprite.Sprite):
    def __init__(self, pos, groups, obstacle_sprites, particle_groups, shake_func, on_death_callback):
    ...
    # --- ИСПРАВЛЕНО: ПРИНИМАЕТ СМЕЩЕНИЕ КАМЕРЫ ---
    def get_mouse_angle(self, camera_offset):
        mx, my = pygame.mouse.get_pos()
        
        # Переводим позицию игрока из МИРОВЫХ координат в ЭКРАННЫЕ
        # Экранная позиция = Мировая позиция + Смещение камеры
        screen_x = self.pos.x + camera_offset.x
        screen_y = self.pos.y + camera_offset.y
        
        # Считаем разницу между мышью и игроком на экране
        dx = mx - screen_x
        dy = my - screen_y
    ...
    def shoot(self):
        ...
        # Прицеливание уже настроено в update_custom -> self.angle
        direction = pygame.math.Vector2(math.cos(math.radians(self.angle)), math.sin(math.radians(self.angle))).normalize()
    ...
    # --- ИСПРАВЛЕНО: ДОБАВЛЕН ARGUMENT camera_offset ---
    def update_custom(self, dt, enemies_group, bullet_group, all_sprites_group, camera_offset):
