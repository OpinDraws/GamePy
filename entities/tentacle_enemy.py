...
class TentacleEnemy(BaseEnemy):
    def __init__(self, pos, player, groups, particle_groups, shake_func):
        # Инициализируем базу (здоровье 80)
        super().__init__(pos, player, groups, particle_groups, shake_func, health=80)
        
        self.radius = 20 
        self.base_radius = random.randint(24, 28)
        
        # Индивидуальная скорость поворота
    ...
    def take_damage(self, amount):
        # Специфическая реакция: при получении урона сразу фокусируется на игроке
        self.attention_state = 'FOCUS'
        self.attention_timer = 50 
        # Вызов базовой логики (уменьшение ХП, частицы, смерть)
