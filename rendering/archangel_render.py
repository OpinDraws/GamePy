...
def draw_smite_sign(surface, center, progress):
    """
    ...
    # 1. Расчет Альфа-канала для плавного появления/исчезновения
    P_START_FADE = (60 + 30 + 15) / (60 + 30 + 15 + 30) # 105/135 ≈ 0.777
    P_END_FADE = 1.0
    FADE_RANGE = P_END_FADE - P_START_FADE # 30 / 135 ≈ 0.222
    
    # Появление (fade-in): первые 10% общей атаки (нарастание)
    FADE_IN_END = 0.1
    fade_in = min(1.0, progress / FADE_IN_END) 
    
    # Исчезновение (fade-out): от P_START_FADE до P_END_FADE
    if progress > P_START_FADE:
        # Прогресс внутри фазы исчезновения (от 0.0 до 1.0)
        fade_out_progress = (progress - P_START_FADE) / FADE_RANGE
        fade_out = max(0.0, 1.0 - fade_out_progress) # Инвертируем: 1.0 -> 0.0
    else:
        fade_out = 1.0 # Держимся на 1.0 до начала исчезновения
        
    # Общий альфа-множитель
    base_alpha_mult = min(fade_in, fade_out) 
    
    # Пульсация (для динамики)
    ...
    draw_alpha_circle(surface, C_CYAN_GLOW[:3] + (glow_alpha,), sign_pos, 15 * base_alpha_mult)
    
    # Основной символ (крест/звезда)
    color = C_CYAN_BRIGHT[:3] + (alpha,)
...
def draw_spear_projectile(surface, center_pos, angle_deg, alpha=255):
    # Улучшенный рендеринг для снарядов (более выраженный градиент/свечение)
    ...
    # 2. Наконечник
    tip_perp = pygame.math.Vector2(-spear_dir.y, spear_dir.x) * 6 # Половина ширины
    tip_poly = [
        tip_center + spear_dir * 20, 
        tip_center - spear_dir * 5 - tip_perp, 
        tip_center - spear_dir * 2, # База наконечника
        tip_center - spear_dir * 5 + tip_perp  
    ]
    ...
    # Добавляем спиральный/градиентный эффект
    num_segments = 5
...
def draw_hand_fist(surface, pos, color, is_left_hand=True, angle_deg=0):
    """Рисует кулак (для правой руки, держащей щит)."""
    angle_rad = math.radians(angle_deg)
    knuckles_w = 10
    knuckles_h = 11
...
def draw_hand_holding_spear(surface, hand_pos, spear_segment_start, spear_segment_end, is_left_hand=True, angle_deg=0):
    """Рисует более реалистичную кисть, обхватывающую древко копья (упрощенная масса пальцев)."""
    angle_rad = math.radians(angle_deg)
    
    spear_vec = (spear_segment_end - spear_segment_start).normalize()
    perp_vec = pygame.math.Vector2(-spear_vec.y, spear_vec.x)
    
    # 1. Основная масса пальцев (единый сегмент, поверх древка)
    ...
    # Рисуем толстую линию, имитирующую 4 сжатых пальца (ширина 7px)
...
def draw_arm_sleeve_only(surface, shoulder_pos, elbow_pos, hand_pos, bend_right=True, is_left_hand=True):
    """Рисует только рукав, не рисуя кисть."""
    
    # 1. Рукав 
    draw_bone_segment(surface, shoulder_pos, elbow_pos, C_WHITE_BASE, 8, 6) # Верхняя рука
    draw_bone_segment(surface, elbow_pos, hand_pos, C_WHITE_BASE, 6, 8) # Предплечье
    ...
    perp_f = pygame.math.Vector2(-(hand_pos-elbow_pos).y, (hand_pos-elbow_pos).x).normalize() * 3 # Уменьшен
...
def calculate_ik_points(shoulder_pos, hand_pos, bend_right=True):
    len_upper = 24
    len_fore = 22
    elbow_pos, hand_ik_pos = solve_ik_2joint(shoulder_pos, hand_pos, len_upper, len_fore, bend_right=False) # Левая рука всегда "влево"
    return shoulder_pos, elbow_pos, hand_ik_pos


# --- Основные функции отрисовки (остальные) ---

def draw_layered_wing(surface, root_pos, angle_deg, scale, time_ticks, flip=False):
    angle_rad = math.radians(angle_deg)
    breath = math.sin(time_ticks * 0.03) * 0.1
    side_mult = 1 if not flip else -1
    
    tip_angle = angle_rad + (0.5 * side_mult) + (breath * 0.5 * side_mult)
    end_pos = root_pos + pygame.math.Vector2(math.cos(tip_angle), math.sin(tip_angle)) * scale * 3.0
    
    ctrl_angle = angle_rad + (1.6 * side_mult) 
    ctrl_dist = scale * 1.5 
...
# --- ОБНОВЛЕНО: Добавлен wing_spread_factor и logic для позы перехода ---
def draw_archangel_boss(surface, center_pos, time_ticks, spear_animation_progress, is_mist_active, mist_timer, fixed_target_pos, smite_vfx_progress=0.0, shield_animation_progress=0.0, is_phase_two=False, wing_spread_factor=0.0, is_transitioning=False): 
    """
    Отрисовка босса.
    ПРИМЕЧАНИЕ: center_pos уже включает смещение тряски экрана.
    """
    center = pygame.math.Vector2(center_pos)
    
    # Усиленное парение во время перехода
    hover_amp = 8
    if is_transitioning:
        hover_amp = 12
    ...
    # --- АНИМАЦИЯ КРЫЛЬЕВ ---
    # Плавный переход угла на основе wing_spread_factor (0.0 -> 1.0)
    # Размах становится шире (угол уменьшается/становится более горизонтальным)
    # Базовый угол: 230. Целевой угол: 200 (-30 градусов).
    spread_angle_offset = -30 * wing_spread_factor 
    
    # Размер не меняем (scale=1.0), только угол
    draw_layered_wing(surface, wing_root, 230 + spread_angle_offset, 50, time_ticks, flip=False) 
    draw_layered_wing(surface, wing_root, -50 - spread_angle_offset, 50, time_ticks, flip=True)  
    
    draw_layered_wing(surface, draw_pos + (0, shoulder_height + 20), 200 + spread_angle_offset, 70, time_ticks + 10, flip=False) 
    draw_layered_wing(surface, draw_pos + (0, shoulder_height + 20), -20 - spread_angle_offset, 70, time_ticks + 10, flip=True)  
    ...
    draw_equipment_and_arms_custom(
        ...
        is_transitioning=is_transitioning, transition_factor=wing_spread_factor
    )
    
    # 5. Индикатор Smite
    if smite_vfx_progress > 0.0: 
        draw_smite_sign(surface, draw_pos, smite_vfx_progress)
...
def draw_equipment_and_arms_custom(surface, body_center, shoulder_l, shoulder_r, time_ticks, spear_animation_progress, shield_animation_progress, is_transitioning=False, transition_factor=0.0):
    ...
    # Если переход, смешиваем с позой перехода
    if is_transitioning:
    ...
    if is_transitioning: glow_active = transition_factor # Светится при переходе
    ...
    TRANSITION_SPEAR_ANGLE = -100 # Почти вертикально
    
    # Смешиваем угол
    current_angle = DEFAULT_SPEAR_ANGLE_DEG + (RAISED_SPEAR_ANGLE_DEG - DEFAULT_SPEAR_ANGLE_DEG) * smooth_progress
    if is_transitioning:
        # Линейная интерполяция углов не всегда работает через 0/360, но тут диапазоны близкие
    ...
    # *** ОБНОВЛЕННАЯ ЛОГИКА: ЩИТ СЛЕДУЕТ ЗА РУКОЙ ***
    
    # 1. Определяем целевые позиции РУКИ (а не щита)
    ...
    # 2. Кривая анимации SmoothStep для плавного возврата
    # t = shield_animation_progress (0.0 to 1.0)
    # Функция: y = t * t * (3 - 2 * t)
    t = shield_animation_progress
    shield_smooth = t * t * (3 - 2 * t)
    
    # 3. Вычисляем позицию руки
    ...
    # 6. Прикрепляем щит к кисти
    # Смещение щита относительно кисти (наружу)
